...
* **Offline Agent Configuration Preflight:** `agent_config.py` owns parsing and validation for the OpenAI-compatible runner. `ai-factory-agent openai-compatible --check-config` validates configuration before reading a prompt or contacting a provider, and the issue workflow runs it before creating the kind cluster. Runtime checks must invoke the preflight with an isolated placeholder API key so ambient secrets cannot mask test failures.
...
* **Agent Runner Image:** The agent runner image is built from `components/agent-sandbox-images/coding-agent/`. It is provider-neutral and runs in any Kubernetes cluster. (The earlier `images/ai-factory-agent/` path was superseded by the sandbox image and has been removed.)
